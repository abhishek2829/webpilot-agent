"""WebPilot Agent — Sales Agent.

Automates outbound sales workflows via browser automation:
  - LinkedIn outreach (connect, message, track)
  - CRM data entry (HubSpot, Pipedrive contacts)
  - Email campaign setup (Instantly, Lemlist)
  - Meeting scheduling (Calendly configuration)
...
class SalesAgent:
    """Sales agent — automates outbound sales and CRM workflows."""

    def __init__(self, workflows_dir: Path = Path("./workflows/sales")) -> None:
    ...
    def description(self) -> str:
        return (
            "Automates outbound sales — LinkedIn outreach, CRM data entry, "
            "email campaign setup, meeting scheduling, and proposal generation."
        )
    ...
    async def execute(self, task: AgentTask) -> AgentResult:
        task_id = task.task_id or f"sales-{uuid4().hex[:12]}"
