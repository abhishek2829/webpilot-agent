"""WebPilot Agent — Marketing Agent.

Automates marketing workflows via browser automation:
  - Social media posting (Twitter/X, LinkedIn posts)
  - SEO audit (analyze rankings, meta tags, competitors)
  - Blog outline generation from research
  - Newsletter/email platform setup (Mailchimp, ConvertKit)
  - Analytics report extraction (Google Analytics, Plausible)
"""
...
class MarketingAgent:
    """Marketing agent — automates content, SEO, and social media workflows."""

    def __init__(self, workflows_dir: Path = Path("./workflows/marketing")) -> None:
    ...
    def description(self) -> str:
        return (
            "Automates marketing — social media posting, SEO audits, "
            "blog outlines, newsletter setup, and analytics report extraction."
        )

    @property
    def capabilities(self) -> list[AgentCapability]:
        return sorted([AgentCapability.BROWSER_AUTOMATION, AgentCapability.FORM_FILLING, AgentCapability.DATA_EXTRACTION], key=lambda c: c.value)
    ...
    async def execute(self, task: AgentTask) -> AgentResult:
        task_id = task.task_id or f"marketing-{uuid4().hex[:12]}"
